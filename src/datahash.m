...
%            'matfile': [Data] is treated as file name of Matlab saved
%                       variables (.mat extension). The header (first 116
%                       bytes) is skipped, because it might contain
%                       different save times although the variables are the
%                       same
%            Default: 'array'.
%
% OUTPUT:
...
if nArg == 2
   ...
   if isfield(Opt, 'Input') && ~isempty(Opt.Input)      % Short-circuiting
      if strcmpi(Opt.Input, 'File')
         ...
         isFile = true;
      elseif strcmpi(Opt.Input, 'matfile')
         if ischar(Data) == 0
            Error_L('CannotOpen', '1st input FileName must be a string');
         end
         isMatFile = true;   
...
elseif isMatFile
   ...
   while Count == Chunk
      [Data, Count] = fread(FID, Chunk, '*uint8');
      if headRead
         Data = Data(117:end,1); % Skip the Matlab file header
         headRead = false;
      end
      if Count ~= 0  % Avoid error for empty file
         Engine.update(Data);
